...
class LevelC:
    def __init__(self, orig_chosen_triplet_indices: List[int], level_a: Any) -> None:
        self.orig_chosen_triplet_indices = list(orig_chosen_triplet_indices)
        # initialize triplets
        T = len(level_a.triplets_abc)
        self.triplets: List[TripletInfo] = [TripletInfo(level_a.triplets_abc[t], 0) for t in range(T)]
        
        # initialize groups
        G = len(level_a.groups)
        self.groups: List[GroupInfo] = [GroupInfo (level_a.group_cardinality[g]) for g in range(G)]
    ...
    class Node:
        __slots__ = ('parent', 'prev', 'triplet_index', 'is_add', 'infeas_count', 'extra_items', 'group_left', 'triplet_used_count')

        def __init__(self, parent: LevelC, prev: Optional [LevelC. Node]=None, is_add: bool=False, triplet_index: int=0) -> None:
            self.parent = parent
            self.prev = prev
        ...
        def _lt_(self, other: LevelC. Node) -> bool:
            ...
            return self.group_left < other.group_left
    ...
    def perform(self, max_infeas_count: int, max_steps: int) -> Tuple [bool, int]: 
        ...
        def key(n: LevelC.Node):
            return (n.infeas_count, -n.extra_items, frozenset(n.group_left.items()), frozenset(n.triplet_used_count.items()))
    ...
    def getFinalTripletIndices(self) -> List[int]:
        return [t for t, info in enumerate(self.triplets) for _ in range(info.used_count)]
