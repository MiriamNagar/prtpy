...
class LevelB:
    ...
    class ExcludeTriplet(TripletBaseStep): 
        def perform(self) -> None:
            print("in perform in ExcludeTriplet")
    ...
    class BranchingStep(Step):
        def __init__(self, options: deque['LevelB.Step'], level_b: Optional['LevelB'] = None)-> None:
            super().__init__(level_b)
    ...
    def __init__(self, a_index_set: List[int], level_a: 'LevelA'):
        ...
        self.stats = LevelB.Stats(Solver.MAX_LOOPS - level_a.answer.total_loops, level_a.t_solver_start)
    ...
    def prepare(self):
        ...
        # groups
        # G = len(self.level_a.groups)
        self.groups: list [GroupInfo] = [GroupInfo(left_a=0, left_bc=self.level_a.group_cardinality[g]) for g in range(G)]
        ...
        for t in range(T):
            ...
            if self.triplets[t].equal_bc: continue
            ...
            #     equal_bc=(b == c),
            #     used_count=0,
            #     index_of_available=len(self.available_triplet_indices),
            #     index_of_available_a=len(self.groups[a].available_triplet_indices_a),
            #     index_of_available_b=len(self.groups[b].available_triplet_indices_bc)
            # )
            # if not info.equal_bc:
            #     info.index_of_available_c = len(self.groups[c].available_triplet_indices_bc)
            #     self.groups[c].available_triplet_indices_bc.append(t)
            # self.triplets.append(info)
            # self.available_triplet_indices.append(t)
            # self.groups[a].available_triplet_indices_a.append(t) 
            # self.groups[b].available_triplet_indices_bc.append(t)
        ...
        for step in self.stack:
            if isinstance(step, self.ApplyTriplet):
                ...
                if step.backtrack_happened:
                    print(f"{step.step_count_start}-{step.step_count_backtrack} | "
    ...
    def execute_backtrack (self) -> bool:
        ...
        def check_next_event():
            if self.stats.current_loops == next_event_handling:
                if self.stats.current_loops == self.stats.max_loops:
                    raise Solver.ProblemTooBig(f"Timeout: branching loop count: {self.stats.current_loops}")
        ...
        while True:
            ...
            try:
                ...
                if self.stats.current_step_count == orig_step_count and self.stats.current_branching_count == orig_branching_count:
                    raise Solver.NoSolution("No changes added to stack in branching loop.")
            # except Solver.NoSolution:
                ...
                # self.stats.loop_states_by_depth[depth] = self.stats.loop_states_by_depth.get(depth, 0) + 1
                # self.stats.loop_states_by_step_counts[steps] = self.stats.loop_states_by_step_counts.get(steps, 0) + 1 
    ...
    def main_branching_loop (self):
        ...
        while True:
            ...
            for triplet_index in list(self.available_triplet_indices):
                a, b, c = self.triplets[triplet_index].triplet
                if (self.take_options_a[a] == self.groups[a].left_a or
                    self.take_options_bc[b] == self.groups[b].left_bc or
                    self.take_options_bc[c] == self.groups[c].left_bc):
                    max_uses = self.get_max_uses_for_triplet_index(triplet_index)
                    for _ in range(max_uses):
                        self.add_step(LevelB.ApplyTriplet(triplet_index, self))
                        if LevelB.ApplyTriplet(triplet_index, self) == None:
                            print("hiiiiiiiiiiiiii")
        ...
        best_ratio= (0,1)
        for a in range(G):
            if not self.groups[a].left_a: continue
        ...
        for bc in range(G):
            if not self.groups[bc].left_bc: continue
        ...
        # addition
        ##########
        triplet_candidate_indices = (self.groups[best_group_index].available_triplet_indices_a 
                                     if best_group_which==1 else 
                                     self.groups[best_group_index].available_triplet_indices_bc) #############
        ...
        for ti in triplet_candidate_indices:
            ...
            if mu> best_mu or (mu == best_mu and ti < best_ti):
                best_mu, best_ti = mu, ti
        if best_mu == 0:
            raise self.BranchImpossible()
            # raise Solver.NoSolution("No triplet to apply.")
        
        # branching apply then exclude
        steps = [LevelB.ApplyTriplet(best_ti, self), LevelB.ExcludeTriplet(best_ti, self)]
        self.stats.winning_branches.append(Solver.BranchingChoiceStats(True, best_group_which==1, best_ratio[0], best_ratio[1], best_mu))
        for step in steps:
            if step == None:
                    print("hiiiiiiiiiiiiii")
...
class LevelA:
    ...
    def get_max_triplet_usage(self, t: Tuple [int, int, int], cardinalities: List[int] = None):
        ...
        if a == b:
            ...
            # counts = sorted([cardinalities[a], cardinalities[b], cardinalities[c]])
            # return min(counts[0] // 2, counts[2])
        # else:
        #     return min(cardinalities[a], cardinalities[b], cardinalities[c])
    ...
    def preprocess(self):
        ...
        while True:
            ...
            for group in range(G):
                if self.group_cardinality[group] == 0: continue
                if len(occurrences[group]) == 0:
                    raise Solver.NoSolution(f"Weight cannot be part of triplet: {self.weight_of_group[group]} for desired sum {self.desired_sum}")
    ...
    def calculate_possibles (self):
        ...
        for g in range(G):
            ...
            if definitely_a < 0 or maybe_a < 0:
                raise Solver.NoSolution("Implementation error: negative definitely_a or maybe_a")
            if definitely_a > 0:
        ...
        for g in range(G):
            ...
            if def_a_left or maybe_a_left:
                raise Solver.NoSolution("Implementation error: not all definitely_a or maybe_a used")
    ...
    def perform_backtrack_level(self):
        ...
        # for bi_int in range(int(self.answer.a_index_set_case_count.to_decimal())):
            ...
            a_index_set += self.mc_maybe_a.get_single_choice(self.answer.maybe_a_choose, case_index)
            ...
            for depth, count in stats.loop_states_by_depth.items():
                self.answer.total_loop_states_by_depth[depth] = self.answer.total_loop_states_by_depth.get(depth, 0) + count
            for stepcount, count in stats.loop_states_by_step_counts.items():
                self.answer.total_loop_states_by_step_counts[stepcount] = self.answer.total_loop_states_by_step_counts.get(stepcount, 0) + count
    ...
    #     final_triplets = self.preprocess_triplets + self.algorithm_chosen_triplets
    #     groups_copy = [deque(g) for g in self.groups]  
    ...
    #         try:
    #             i = groups_copy[a].popleft() 
    #             j = groups_copy[b].popleft() 
    #             k = groups_copy[c].popleft()
    #         except IndexError:
    #             raise Solver.NoSolution ("Index error in group copy during finalize.")
            
    #         solution.add(Triplet(self.orig_weights[i], self.orig_weights[j], self.orig_weights[k]))
    ...
    def finalize_solution(self):
        ...
        def get_next_index(group_index):
            ...
            if not current_group:
                raise Solver.NoSolution("Implementation error: current group already empty.")
